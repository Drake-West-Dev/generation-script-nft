...
def generate_images(edition, count):
    ...
    rarity_table_initial = {}
    for layer in CONFIG:
        rarity_table_initial[layer["name"]] = []
    ...
    for n in progressbar(range(count)):
        ...
        for idx, trait in enumerate(trait_sets):
            if trait is not None:
                rarity_table_initial[CONFIG[idx]["name"]].append(
                    trait[: -1 * len(".mp4")]
                )
            else:
                rarity_table_initial[CONFIG[idx]["name"]].append("none")

    # Create the final rarity table by removing duplicate creat
    rarity_table_pd = pd.DataFrame(rarity_table_initial)
...
def main(num_nfts, nft_name):
    ...
    print("Generated %i images, %i are distinct" % (num_nfts, rarity_table.shape[0]))

    rarity_table_no_dups = drop_duplicates(
        nft_name, num_nfts, zfill_count, rarity_table
    )
